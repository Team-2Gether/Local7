...
CREATE TABLE TB_RESTAURANT (
    ...
    AVERAGE_RATING NUMBER(2, 1) DEFAULT 0,          --추가(평균 점수)
    TOTAL_COMMENTS NUMBER DEFAULT 0;                -- 추가(댓글 개수)
    CREATED_DATE TIMESTAMP DEFAULT SYSTIMESTAMP,
    CREATED_ID VARCHAR2(100),
    UPDATED_DATE TIMESTAMP DEFAULT SYSTIMESTAMP,
    UPDATED_ID VARCHAR2(100),
    TB_RESTAURANT KAKAO_PLACE_ID VARCHAR2(50);
);
...
COMMENT ON COLUMN TB_RESTAURANT.AVERAGE_RATING IS '음식점 평균 평점';
COMMENT ON COLUMN TB_RESTAURANT.TOTAL_COMMENTS IS '음식점 리뷰 총 개수';
...
CREATE TABLE TB_REGION (
    REGION_ID NUMBER NOT NULL PRIMARY KEY,       -- 지역 고유 ID (기본 키)
    REGION_NAME VARCHAR2(100) NOT NULL UNIQUE,   -- 지역 이름 (예: 서울, 부산, 강남구 등)
    REGION_DESCRIPTION CLOB,                     -- 지역 설명 (긴 텍스트를 위해 CLOB 사용)
    VIEW_COUNT NUMBER DEFAULT 0,                 -- 조회수 (기본값 0)
    CREATED_DATE TIMESTAMP DEFAULT SYSTIMESTAMP, -- 생성 일시
    CREATED_ID VARCHAR2(100),                    -- 생성 사용자 ID
    UPDATED_DATE TIMESTAMP DEFAULT SYSTIMESTAMP, -- 수정 일시
    UPDATED_ID VARCHAR2(100)                     -- 수정 사용자 ID
);

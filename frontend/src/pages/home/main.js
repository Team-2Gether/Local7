    ...
    const initializeMapAndLoadData = () => {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                new window.kakao.maps.Marker({ map, position: locPosition, title: '현재 위치' });
            });
        }
                ...
                if (response.data?.status === 'success') {
                    ...
                    fetched.forEach((r) => {
                        if (r.restaurantLat && r.restaurantLon) {
                            ...
                            new window.kakao.maps.Marker({ map, position: pos, title: r.restaurantName });
                            bounds.extend(pos);
                        }
                    });
            ...
            <Routes>
                ...
                <Route path="/restaurants" element={
                    <div className="home-container">
                        <h1>환영합니다, {currentUser ? currentUser.userNickname : '게스트'} 님!</h1>
                        ...
                        {restaurants.length > 0 ? (
                            <ul className="restaurant-list">
                                {restaurants.map((r) => {
                                    ...
                                    return (
                                        <li key={r.restaurantId} className="restaurant-item" onClick={() => handleRestaurantClick(r)}>

...
function useLogin(onLoginSuccess) {
    const { formData, handleChange } = useFormData({
        credential: '',
        password: '',
    });//useFormData를 사용하여 formData와 handleChange를 초기화
    ...
    return {
        ...
        setCredential: (value) => handleChange({ target: { name: 'credential', value } }),//handleChange를 사용하여 credential 업데이트
        password: formData.password,//password를 formData.password로 변경
        setPassword: (value) => handleChange({ target: { name: 'password', value } }),//handleChange를 사용하여 password 업데이트
        handleSubmit,
    };
}
